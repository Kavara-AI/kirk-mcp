...
EXPECTED_SHA = os.environ.get(
    "KIRK_EXPECTED_SHA",
    "f3c548477d3b5c7612ef1863b06293d3a922c57926738426bc75ab64720de5ec",
)
...
MOCK_SHA = "f3c548477d3b5c7612ef1863b06293d3a922c57926738426bc75ab64720de5ec"


def call_tool(name: str, arguments: dict):
    if MOCK_MODE:
        time.sleep(0.01)
        return 0.01, {"result": {"engine_sha": MOCK_SHA}}, MOCK_SHA
...
def main():
    ...
    parser.add_argument(
        "--mock", action="store_true", help="Run with synthetic safe local inputs"
    )
    ...
    try:
        ...
        if actual_sha and actual_sha != EXPECTED_SHA:
            print(
                f"[FAIL] Provenance mismatch! Expected: {EXPECTED_SHA}, got: {actual_sha}"
            )
            failed = True
        else:
    ...
    def worker(i):
        try:
            return call_tool(
                "kirk_score_random",
                {"n": 10, "model_id": "kirk-fy24-log-return-cascade-v1"},
            )
        except Exception:
            return None
        ...
        for f in concurrent.futures.as_completed(futures):
            ...
            if res:
                ...
                if dur <= MAX_LATENCY_S:
                    ...
                    print(
                        f"[WARN] Request exceeded max latency: {dur:.4f}s > {MAX_LATENCY_S}s"
                    )

    print(
        f"Concurrency results: {successes}/{TARGET_CONCURRENCY} succeeded within {MAX_LATENCY_S}s."
    )
    if successes < TARGET_CONCURRENCY:
        print("[FAIL] Concurrency latency gate failed.")
        failed = True
    else:
